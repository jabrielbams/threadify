...
import { Shield, Flag, Scale, LogOut, CheckCircle, XCircle, Clock } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  async function handleAppealAction(appealId: string, action: "approved" | "rejected") {
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-4xl px-4 py-6">
        ...
        <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
          <StatCard label="Banding Menunggu" value={pendingAppeals} icon={Scale} color="text-primary" />
          <StatCard label="Laporan Menunggu" value={pendingReports} icon={Flag} color="text-tertiary" />
          <StatCard label="Total Banding" value={appeals.length} icon={Scale} color="text-on-surface-variant" />
          <StatCard label="Total Laporan" value={reports.length} icon={Flag} color="text-on-surface-variant" />
        </div>
              ...
              appeals.map((appeal) => (
                ...
                >
                  ...
                  <div className="mb-3 flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-label-xs text-on-surface-variant">
                        @{appeal.profiles?.username ?? "unknown"} · {formatRelativeTime(appeal.created_at)}
                      </p>
                    </div>
                  ...
                  {appeal.strikes && (
                    <div className="mb-3 rounded-lg bg-surface-container p-3">
                      <div className="flex items-center justify-between text-label-xs text-on-surface-variant">
                        <span>Strike #{appeal.strikes.strike_number} · {appeal.strikes.content_type === "post" ? "Postingan" : "Komentar"}</span>
                        <span>AI: {Math.round((appeal.strikes.ai_confidence ?? 0) * 100)}%</span>
                      </div>
                  ...
                  <div className="mb-3">
                    ...
                    <p className="mt-1 text-body-sm text-on-surface">{appeal.reason}</p>
                  </div>
              ...
              reports.map((report) => (
                ...
                >
                  ...
                  <div className="mb-3 flex items-start justify-between">
                    <div>
                      <p className="text-label-md font-bold text-on-surface">
                        Dilaporkan oleh @{report.profiles?.username ?? "unknown"}
                      </p>
                      <p className="text-label-xs text-on-surface-variant">
                  ...
                  {report.description && (
                    <div className="mb-3">
                      ...
                      <p className="mt-1 text-body-sm text-on-surface">{report.description}</p>
                    </div>
                  )}
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; cls: string }> = {
    ...
    dismissed: { label: "Diabaikan", cls: "bg-surface-container text-on-surface-variant" },
    reviewed: { label: "Ditinjau", cls: "bg-primary-fixed text-primary" },
  };
  const c = config[status] ?? { label: status, cls: "bg-surface-container text-on-surface-variant" };

  return (
    <span className={cn("rounded-full px-2.5 py-1 text-label-xs font-medium", c.cls)}>
      {c.label}
    </span>
  );
}
